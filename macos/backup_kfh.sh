...
# `--omit-dir-times`: This option tells `rsync` to skip updating the timestamps of directories when syncing. By default,
#                    `rsync` updates the modification times of directories, but this option prevents that from happening.

# `--delete`: This option tells `rsync` to delete files in the destination directory that are not present in the source directory. 
#             This is useful for ensuring that the destination is an exact mirror of the source.
...
# * Feb.15, 2025 : Added the doc on the "rsync" options.
